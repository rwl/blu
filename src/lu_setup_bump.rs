...
pub(crate) fn lu_setup_bump(
    ...
    let Lnz = this.factor.Lbegin_p[rank as usize] - rank;
    let Unz = this.factor.Ubegin[rank as usize];
    ...
    let pinv = &this.factor.pinv;
    let qinv = &this.factor.qinv;
    // let Wbegin = &mut this.Wbegin;
    // let Wend = &mut this.Wend;
    // let Wbegin2 = Wbegin + m; /* alias for row file */
    // let Wend2 = Wend + m;
    let (Wbegin, Wbegin2) = this.factor.Wbegin.split_at_mut(m as usize);
    let (Wend, Wend2) = this.factor.Wend.split_at_mut(m as usize);
    ...
    let iwork0 = &mut this.factor.iwork0;
